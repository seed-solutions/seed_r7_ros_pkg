...
void seed_converter::TypeGArm::makeTables()
{
...
void seed_converter::TypeGArm::Angle2Stroke
(std::vector<int16_t>& _strokes, const std::vector<double>& _angles)
{
  ...
  _strokes[2]
    = static_cast<int16_t>(scale * setAngleToStroke(rad2Deg * _angles[2], elbow_p.table));
...
void seed_converter::TypeGArm::Stroke2Angle
(std::vector<double>& _angles, const std::vector<int16_t>& _strokes)
{
  ...
  _angles[2] = deg2Rad * setStrokeToAngle(scale_inv * _strokes[2], elbow_p.inv_table);
...
PLUGINLIB_EXPORT_CLASS(seed_converter::TypeGArm, seed_converter::StrokeConverter)
