...
NoidMoverController::NoidMoverController(const ros::NodeHandle& _nh, noid_robot_hardware::NoidRobotHW *_in_hw) :
  nh_(_nh),hw_(_in_hw),
  vx_(0), vy_(0), vth_(0), x_(0), y_(0), th_(0)//, base_spinner_(1, &base_queue_)
{
  ...
  nh_.getParam("noid_mover_controller/wheel_radius", wheel_radius);
  nh_.getParam("noid_mover_controller/tread", tread);
  nh_.getParam("noid_mover_controller/wheelbase", wheelbase);

  k1_ =  - sqrt(2) * ( sqrt( pow(tread,2)+pow(wheelbase,2) )/2 ) * sin( M_PI/4 + atan2(tread/2,wheelbase/2) ) / wheel_radius;
  k2_ = 1 / wheel_radius;
  //---------

  nh_.getParam("noid_mover_controller/ros_rate", ros_rate_);
  nh_.getParam("noid_mover_controller/odom_rate", odom_rate_);
  nh_.getParam("noid_mover_controller/safety_rate", safety_rate_);
  nh_.getParam("noid_mover_controller/safety_duration", safety_duration_);
...
*/
  ...
  cmd_vel_sub_ = nh_.subscribe("/cmd_vel",1, &NoidMoverController::cmdVelCallback,this);
  safe_timer_ = nh_.createTimer(ros::Duration(safety_rate_), &NoidMoverController::safetyCheckCallback, this);

  // for odometory
  odom_pub_ = nh_.advertise<nav_msgs::Odometry>("odom", 1);
  odom_timer_ = nh_.createTimer(ros::Duration(odom_rate_), &NoidMoverController::calculateOdometry, this);
}
...
void NoidMoverController::velocityToWheel(double _linear_x, double _linear_y, double _angular_z, std::vector<int16_t>& _wheel_vel) 
{
