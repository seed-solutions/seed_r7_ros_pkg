...
robot_hardware::LowerController::LowerController(const std::string &_port)
{
  ...
  for (size_t i = 0; i < aero_table_.size(); ++i)
  {
    ...
    if (index != aero_index_.size())
      aero_table_.at(i) = std::make_pair(index, name_.at(index));
  }
  ...
  for (size_t i = 0; i < wheel_table_.size(); ++i)
  {
    ...
    if (index != wheel_aero_index_.size())
      wheel_table_.at(i) = std::make_pair(index, wheel_name_.at(index));
  }
}
...
void robot_hardware::LowerController::getPosition()
{
  if (is_open_)
  {
    ...
    {
      wheel_angles_.at(wheel_aero_index_.at(i) - 2) = raw_data_.at(wheel_aero_index_.at(i));
    }
  }
...
void robot_hardware::LowerController::remapAeroToRos(std::vector<int16_t> &_ros, std::vector<int16_t> &_aero)
{
  _ros.resize(name_.size());
  for (size_t i = 0; i < _ros.size(); ++i)
  {
    if (aero_index_.at(i) != -1)
      _ros.at(i) = _aero.at(aero_index_.at(i));
  }
}

void robot_hardware::LowerController::remapRosToAero(std::vector<int16_t> &_aero, std::vector<int16_t> &_ros)
{
...
{
  if (is_open_)
    lower_->runScript(_number, _script);
}
void robot_hardware::LowerController::sendVelocity(std::vector<int16_t> &_data)
{
  ...
  for (size_t i = 0; i < wheel_table_.size(); ++i)
  {
    if (wheel_table_.at(i).second != "")
      send_data.at(i) = _data.at(wheel_table_.at(i).first);
  }

  if (is_open_)
  {
    raw_data_ = lower_->actuateBySpeed(send_data.data());
    for (size_t i = 0; i < wheel_angles_.size(); ++i)
    {
      wheel_angles_.at(wheel_aero_index_.at(i) - 2) = raw_data_.at(wheel_aero_index_.at(i));
    }
